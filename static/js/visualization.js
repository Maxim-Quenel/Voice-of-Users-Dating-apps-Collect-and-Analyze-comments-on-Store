(() => {
  const shareInput = document.getElementById("viz_share");
  const shareValue = document.getElementById("viz_share_value");
  const overlay = document.getElementById("loading-overlay");
  const progressBar = document.getElementById("progress-bar");
  let progressTimer;

  if (shareInput && shareValue) {
  ...
  const chartDataEl = document.getElementById("viz-chart-data");
  let chartData = (window.VIZ_CHART_DATA && typeof window.VIZ_CHART_DATA === "object")
    ? window.VIZ_CHART_DATA
    : { columns: [], sample: [], types: {}, sample_size: 0 };

  if ((!chartData.sample || chartData.sample.length === 0) && chartDataEl && chartDataEl.textContent) {
    try {
  ...
  if (chartData && chartData.sample && !Array.isArray(chartData.sample)) {
    chartData.sample = Object.values(chartData.sample);
  }
  if (chartData && chartData.columns && !Array.isArray(chartData.columns)) {
    chartData.columns = Object.values(chartData.columns);
  }
  ...
  function inferTypesFromSample(sample, columns) {
    const inferred = {};
    ...
    columns.forEach(col => {
      ...
      } else if (dateCount / nonEmpty >= 0.6) {
        inferred[col] = "date";
      } else {
        inferred[col] = "text";
      }
  ...
  function aggregateRows(rows, xCol, yCol, agg, colorCol) {
    ...
    rows.forEach(row => {
      ...
      if (xVal === null) return;
      const colorVal = colorCol ? normalizeValue(row[colorCol], cType) : "Serie";
  ...
  function initChartBuilder() {
    ...
    let columns = chartData.columns || [];
    const sampleKeys = chartData.sample.length ? Object.keys(chartData.sample[0] || {}) : [];
    if (sampleKeys.length) {
      columns = sampleKeys;
    }
    if (!columns.length) return;

    const inferredTypes = inferTypesFromSample(chartData.sample, columns);
    if (!chartData.types) {
      chartData.types = inferredTypes;
    } else {
      columns.forEach(col => {
        if (!chartData.types[col]) {
          chartData.types[col] = inferredTypes[col] || "text";
        }
      });
    }
    ...
    function renderChart() {
      ...
      if (type === "scatter") {
        if (!xCol || !yCol) {
          ...
          error = "Le nuage de points necessite un Y numerique.";
        } else {
        ...
        if (!xCol) {
          ...
          error = "Choisissez un Y numerique pour cette aggregation.";
        } else {
        ...
        if (!xCol) {
          ...
          error = "L'histogramme necessite un X numerique.";
        } else {
        ...
        if (!yCol || getColumnType(yCol) !== "numeric") {
          error = "Le boxplot necessite un Y numerique.";
        } else {
      ...
      if (hintEl) {
        hintEl.textContent = error || "Graphique pret.";
      }
